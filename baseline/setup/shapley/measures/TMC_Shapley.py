...
class TMC_Shapley(Measure):
    ...
    def one_iteration(
        ...
        idxs, marginal_contribs = np.random.permutation(len(sources.keys())), np.zeros(
            X_train.shape[0]
        )
        new_score = np.max(np.bincount(y_test).astype(float) / len(y_test))
        X_batch, y_batch = np.zeros((0,) + tuple(X_train.shape[1:])), np.zeros(
            0
        ).astype(int)
        ...
        for n, idx in enumerate(idxs):
            ...
            marginal_contribs[sources[idx]] = (new_score - old_score) / len(
                sources[idx]
            )
    ...
    def score(
        ...
        for iteration in range(iterations):
            if 10 * (iteration + 1) / iterations % 1 == 0:
                print(
                    "{} out of {} TMC_Shapley iterations.".format(
